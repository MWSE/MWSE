...
namespace NI {
	...
	std::shared_ptr<TES3::BoundingBox> AVObject::createBoundingBox_lua(sol::optional<sol::table> maybeParams) const {
		auto params = maybeParams.value_or(sol::table {});
		auto accuratedSkinned = params.get_or("accurateSkinned", false);
		auto observeAppCullFlag = params.get_or("observeAppCullFlag", false);
		auto onlyActiveChildren = params.get_or("onlyActiveChildren", false);
