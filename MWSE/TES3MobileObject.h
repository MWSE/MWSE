...
namespace TES3 {
	...
	struct MobileObject_vTable {
		...
		void (__thiscall* setPosition)(MobileObject*, NI::Vector3*); // 0x40
		...
		bool (__thiscall* findDropPointOnCollider)(MobileObject*, float, NI::AVObject*, float*, NI::Vector3*); // 0x7C
	...
	struct MobileActor_vTable : MobileObject_vTable {
		...
		bool (__thiscall* dropObjectRayTests)(MobileActor*, float, NI::Node*, float*, NI::Vector3*, unsigned char*); // 0x104
	};
	...
	struct MobileObject {
		struct Collision {
			...
			bool valid; // 0x0
			float time; // 0x4
			NI::Vector3 point; // 0x8
			NI::Vector3 objectPosAtCollision; // 0x14
			NI::Vector3 velocity; // 0x20
		...
		float height; // 0x2C
		NI::Vector2 boundSize; // 0x30
		float simulationDistance; // 0x38
		NI::Vector3 velocity; // 0x3C
		NI::Vector3 impulseVelocity; // 0x48
		NI::Vector3 positionAtLastLightUpdate; // 0x54
		NI::CollisionGroup * collisionGroup; // 0x60
		float thisFrameDistanceMoved; // 0x64
		NI::Vector3 thisFrameDeltaPosition; // 0x68
		...
		void setFootPoint(const NI::Vector3* point);
		void setInstantVelocity(const NI::Vector3* velocity);
		void updateConstantVelocity(const NI::Vector3* velocity);
		...
		NI::Vector3 getBoundSize() const;
		void setBoundSize(const NI::Vector3&);
		NI::Vector3* getImpulseVelocity();
