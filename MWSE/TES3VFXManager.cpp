...
namespace TES3 {
	...
	const auto TES3_VFXManager_createAtPosition = reinterpret_cast<VFX * (__thiscall*)(VFXManager*, unsigned int, PhysicalObject*, NI::Vector3*, int, float, float, float)>(0x468470);
	VFX* VFXManager::createAtPosition(unsigned int serial, PhysicalObject* effect, NI::Vector3* position, int animLoopCount, float lifespan, float scale, float verticalOffset) {
