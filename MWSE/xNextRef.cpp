...
namespace mwse {
	...
	float xNextRef::execute(mwse::VMExecuteInterface& virtualMachine) {
		...
		__except (EXCEPTION_EXECUTE_HANDLER) {
			...
			if constexpr (DEBUG_MWSCRIPT_FUNCTIONS) {
				mwse::log::getLog() << "xNextRef: Invalid object given in script " << script->sourceFile->fileName << "/" << script->header.name << ". Fix script to not save variables across saves!" << std::endl;
			}
			next = nullptr;
		}
