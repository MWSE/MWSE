...
```lua
local element = tes3.messageBox({ message = ..., buttons = ..., callback = ..., showInDialog = ..., duration = ... }, unknown)
```
...
* `...`: *Optional*. Formatting arguments. These are passed to `string.format`, provided `messageOrParams` is a `string`.
