return {
	...
	["tolerateBrokenDialogueScripts.description"] = "If enabled, a broken dialogue result script no longer freezes or crashes the game in the dialogue window. The engine normally shows a script-error box whose message loop keeps redrawing the same broken dialogue, re-triggering the error until the game runs out of stack and crashes; this makes that error box appear at most once instead of recursing. It also makes an AddTopic to a dialogue topic that does not exist skip just that command and keep running the rest of the result, instead of discarding the whole script. This prevents a broken mod (for example an AddTopic to a topic it never created) from crashing the game. On by default.",
}
