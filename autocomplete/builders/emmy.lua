...
---@param package package
---@param file file*
local function buildAliases(package, file)
	if not package.aliases then
		return
	end

	local aliasNames = table.keys(package.aliases, true)
	for _, aliasName in ipairs(aliasNames) do
		file:write(string.format("--- @alias %s\n", aliasName))
		for _, value in ipairs(package.aliases[aliasName]) do
...
local function build(package)
	...
	-- Write out any top-level aliases.
	buildAliases(package, file)
